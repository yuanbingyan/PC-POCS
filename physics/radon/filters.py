...
class AbstractFilter(nn.Module):
    ...
    def forward(self, x):
        ...
        projection = torch.fft.fft(padded_tensor.transpose(2, 3), dim=-1)
        projection = projection * fourier_filter
        result = torch.fft.ifft(projection, dim=-1).real
    ...
    def _get_fourier_filter(self, size):
        ...
        fourier_filter = torch.fft.fft(f, dim=0)
...
class HannFilter(AbstractFilter):
    ...
    def create_filter(self, f):
        ...
        hann = 0.5 - 0.5 * torch.cos(2.0 * PI * n / (f.shape[0] - 1))
...
class LearnableFilter(AbstractFilter):
    ...
    def forward(self, x):
        ...
        """
        fourier_filter = self.filter.to(x.device).unsqueeze(-1)  # [W, 1]
        projection = torch.fft.fft(x.transpose(2, 3), dim=-1)
        projection = projection * fourier_filter
        result = torch.fft.ifft(projection, dim=-1).real
